 ...
 * @internal
 */
export function parseValueOrValuesAsArray(valueOrValues) {
 ...
 * @internal
 */
export function parseParameterizedEndpointPath(endpoint, pathParams) {
    for (const pathParameterKey in pathParams) {
        `${pathParams[pathParameterKey]}`;
        endpoint = endpoint.replaceAll(`{${pathParameterKey}}`, 
    ...
    if (unparsedParameters)
        throw new Error(`Missing path parameter values for ${unparsedParameters.join(" ,")}`);
    return endpoint;
}
