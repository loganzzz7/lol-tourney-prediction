...
def parse_game_window_to_excel(json_path, output_excel_path, blue_code, red_code):
    ...
    def aggregate_team_stats(participants, team_ids):
        ...
        return {
            ...
            "avgKDA": round(
                sum((p["kills"] + p["assists"]) / max(p["deaths"], 1)
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) != 5:
        print("usage: python parse_game_window.py <input.json> <output.xlsx> <blue_code> <red_code>")
    else:
        _, inp, outp, blue_code, red_code = sys.argv
        parse_game_window_to_excel(inp, outp, blue_code, red_code)